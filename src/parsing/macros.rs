macro_rules! remove_overriden {
    ...
    (@arg $parser:ident, $arg:ident) => {
        ...
        remove_overriden!(@remove $parser.conflicts, $arg.conflicts);
        remove_overriden!(@remove $parser.overrides, $arg.overrides);
    };
...
macro_rules! arg_post_processing {
    ($parser:ident, $arg:ident, $matcher:ident) => {
        ...
        if $parser.overrides.contains(&$arg.name) {
            if let Some(ref name) = find_name_from!($parser.app, &$arg.name, overrides, $matcher) {
    ...
    (
        ...
        if !$parser.is_set(AS::TrailingValues) &&
           ($parser.is_set(AS::TrailingVarArg) &&
            ...
            $parser.settings.set(AS::TrailingValues);
        }
        ...
        $parser.settings.set(AS::ValidArgFound);
